# FeedbackPro — теоретическая модель работы с отзывами T2
...
- в главе 2 как эталон для сравнения с AS-IS организации N;
- в главе 3 как основа проектной TO-BE программы FeedbackPro.
...
Один отзыв может иметь несколько аспектов. Состав категорий уточняется по результатам фактического контент-анализа организации N и не должен подгоняться под заранее желаемое распределение.
...
Точные SLA и должностные маршруты определяются в T4 после анализа AS-IS и организационной структуры N.
...
T2 фиксирует роли, но не конкретные должности организации N. Детальная RACI формируется в T4.
...
## 7. Трёхконтурная модель FeedbackPro
...
### Глава 2
Модель становится эталоном сравнения с AS-IS:
- какие этапы отсутствуют;
- где нет владельца;
- где теряется информация;
- есть ли единая классификация;
- есть ли эскалация и контроль;
- как используются агрегированные данные.
